...
function TickerTape({ items, dark }: { items: MacroItem[]; dark: boolean }) {
  ...
  return (
    <div className={`overflow-hidden border-b h-8 flex items-center select-none ${dark ? "border-zinc-800/50 bg-zinc-950" : "border-gray-200 bg-gray-50"}`}>


      <div className="flex animate-ticker whitespace-nowrap">
        {filled.map((m, i) => (
          <span key={i} className="inline-flex items-center gap-2 px-4 text-xs font-mono">
            <span className={`font-semibold tracking-wide ${dark ? "text-amber-400" : "text-amber-600"}`}>{m.label}</span>
            <span className={`font-bold ${m.change_pct != null && m.change_pct > 0 ? "text-emerald-400" : m.change_pct != null && m.change_pct < 0 ? "text-red-400" : dark ? "text-zinc-300" : "text-gray-700"}`}>
              {m.price != null ? fmt(m.price) : "—"}
            </span>
            {m.change_pct != null && <span className={`text-[10px] ${clr(m.change_pct)}`}>{pct(m.change_pct)}</span>}
            <span className={`mx-1 ${dark ? "text-zinc-700" : "text-gray-300"}`}>·</span>
          </span>
        ))}
      </div>
    </div>
...
function MacroCard({ item, onClick, dark }: { item: MacroItem; onClick?: () => void; dark: boolean }) {
  ...
  return (
    <div onClick={onClick} className={`p-3 border rounded-sm transition-all duration-700 cursor-pointer ${
      ...
      <div className={`text-[10px] uppercase tracking-widest mb-1 truncate ${dark ? "text-zinc-500" : "text-gray-500"}`}>{item.label}</div>
...
function CryptoPanel() {
  ...
  return (
    <div style={{ padding: "24px 28px 16px", display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
      <div>
        <div style={{ fontSize: 26, fontWeight: 700, color: "#1a1a2e", fontFamily: "'DM Sans', sans-serif", letterSpacing: "-0.02em" }}>
          {greeting}, Justin
        </div>
        <div style={{ fontSize: 14, color: "#8b92a0", fontFamily: "'DM Sans', sans-serif", marginTop: 4 }}>
          {dateStr}
        </div>
      </div>
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 11, color: "#8b92a0", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.05em" }}>Market Status</div>
          <div style={{ fontSize: 13, fontWeight: 600, color: hour >= 9 && hour < 16 ? "#0d9f4f" : "#8b92a0" }}>
  ...
  useEffect(() => {
    ...
    const fetchE = () => {
      fetch(`${BASE}/photonics/premarket/${ticker}`)
        .then(r => r.json()).then(d => { if (!cancelled) setExt(d); }).catch(() => {});
    };
    fetchQ(); fetchE();
  ...
  const loadPortfolio = useCallback(async () => { try { const r = await fetch(`${BASE}/portfolio/${portfolioName}`); const d = await r.json(); setPortfolio(d.holdings||{}); } catch {} }, []);
  ...
  const loadJournal = useCallback(async () => { try { const r = await fetch(`${BASE}/journal`); const d = await r.json(); setTrades(d.trades||[]); setTradeStats(d.stats||{}); } catch {} }, []);
  ...
  const loadWlPrices = useCallback(async (tks: string[]) => { if (!tks.length) return; try { const r = await fetch(`${BASE}/quotes?tickers=${tks.join(",")}`); const d = await r.json(); const m:{[t:string]:number|null}={}; (d.data||[]).forEach((q:any)=>{m[q.ticker]=q.price??null;}); setWlPrices(m); } catch {} }, []);
  ...
  // ── Init ────────────────────────────────────────────────────────────
  useEffect(() => {
    loadMacro(); loadNews(); loadBreaking(); loadCalendar(); loadWatchlist();
    loadAlerts(); loadPortfolio(); loadConvictions(); loadKeywords();
    loadNewsAlerts(); loadJournal(); loadEarnings(); loadRhStatus(); loadFidAccounts(); loadMmStatus();
    const t1 = setInterval(loadMacro, 30000);
    const t2 = setInterval(() => { loadNews(); loadBreaking(); loadNewsAlerts(); }, 60000);
    const t3 = setInterval(loadAlerts, 30000);
    return () => { clearInterval(t1); clearInterval(t2); clearInterval(t3); };
  }, []);
  useEffect(() => { if (watchlist.length) loadWlPrices(watchlist); }, [watchlist]);
  ...
  const portfolioRows = Object.entries(portfolio).map(([ticker, h]) => {
    ...
    return { ticker, ...h, price, mktVal, cost, pnl, pnlPct };
  });
  ...
  const addAlert = async () => { if(!alTicker||!alPrice) return; await fetch(`${BASE}/alerts`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({ticker:alTicker.toUpperCase(),condition:alCond,price:parseFloat(alPrice)})}); setAlTicker(""); setAlPrice(""); loadAlerts(); };
  const deleteAlert = async (id:string) => { await fetch(`${BASE}/alerts/${id}`,{method:"DELETE"}); loadAlerts(); };
  const addPortfolio = async () => { if(!ptTicker||!ptShares) return; await fetch(`${BASE}/portfolio/${portfolioName}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({ticker:ptTicker.toUpperCase(),shares:parseFloat(ptShares),cost_basis:parseFloat(ptCost||"0")})}); setPtTicker(""); setPtShares(""); setPtCost(""); loadPortfolio(); };
  const deletePortfolio = async (t:string) => { await fetch(`${BASE}/portfolio/${portfolioName}/${t}`,{method:"DELETE"}); loadPortfolio(); };
  ...
  // Theme classes
  const bg = dark ? "bg-zinc-950 text-zinc-200" : "bg-gray-50 text-gray-900";
  const cardBg = dark ? "bg-zinc-900/50 border-zinc-800/50" : "bg-white border-gray-200 shadow-sm";
  const headerBg = dark ? "bg-zinc-950/80 border-zinc-800/50" : "bg-white/80 border-gray-200";
  const inputCls = dark ? "bg-zinc-900 border-zinc-700/50 text-zinc-200 focus:border-amber-500" : "bg-white border-gray-300 text-gray-900 focus:border-amber-500";
  const input = `${inputCls} px-3 py-2 text-xs focus:outline-none rounded-sm font-mono border`;
  const btn = "bg-amber-400 text-zinc-950 px-4 py-2 text-xs font-bold hover:bg-amber-300 rounded-sm transition-colors";
  const btnOutline = dark ? "border border-zinc-700 text-zinc-400 px-3 py-1.5 text-xs rounded-sm hover:border-amber-500 hover:text-amber-400 transition-colors" : "border border-gray-300 text-gray-600 px-3 py-1.5 text-xs rounded-sm hover:border-amber-500 hover:text-amber-600 transition-colors";
  const dimText = dark ? "text-zinc-500" : "text-gray-500";
  const dimText2 = dark ? "text-zinc-600" : "text-gray-400";
  const dimText3 = dark ? "text-zinc-700" : "text-gray-300";
  const bodyText = dark ? "text-zinc-300" : "text-gray-700";
  const headText = dark ? "text-zinc-100" : "text-gray-900";
  const borderDim = dark ? "border-zinc-800/50" : "border-gray-200";
  const borderDim2 = dark ? "border-zinc-900/50" : "border-gray-100";
  ...
  return (
    <div className={`min-h-screen font-mono flex flex-col text-sm ${bg} transition-colors duration-300`}>
        <style dangerouslySetInnerHTML={{ __html: GLOBAL_STYLES }} />
        <PersonalHeader />
      {macro.length > 0 && <TickerTape items={macro} dark={dark} />}

      <header className={`sticky top-0 z-40 backdrop-blur-xl border-b px-3 sm:px-4 py-2 flex items-center gap-2 ${headerBg}`}>
        <span className="text-amber-400 font-bold tracking-widest text-xs uppercase hidden md:inline">◈ TERMINAL</span>
        <div className={`h-4 w-px hidden md:block ${dimText3}`} />

        {/* Mobile hamburger */}
        <button onClick={()=>setMobileNav(v=>!v)} className="md:hidden text-amber-400 text-lg px-1">☰</button>

        {/* Desktop nav */}
        <nav className="hidden md:flex gap-0.5 flex-wrap overflow-x-auto scrollbar-hide flex-1">
          {tabs.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)}
              className={`px-2 lg:px-3 py-1 text-xs rounded-sm transition-colors whitespace-nowrap ${tab===t.id?"bg-amber-400 text-zinc-950 font-bold":dark?"text-zinc-500 hover:text-zinc-200 hover:bg-zinc-900":"text-gray-500 hover:text-gray-900 hover:bg-gray-100"}`}>
              {t.label}
            </button>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-2 ml-auto shrink-0">
          <button onClick={()=>setDark(d=>!d)} className={`px-2 py-1 rounded-sm text-xs border transition-colors ${dark?"border-zinc-700 text-zinc-400 hover:text-amber-400":"border-gray-300 text-gray-500 hover:text-amber-600"}`}>
            {dark ? "☀️" : "🌙"}
          </button>
          {lastUpdated && <span className={`text-[10px] hidden lg:inline ${dimText2}`}>↻ {lastUpdated}</span>}
        </div>
      </header>

      {/* Mobile nav drawer */}
      {mobileNav && (
        <div className={`md:hidden border-b p-2 grid grid-cols-4 gap-1 ${dark?"bg-zinc-900 border-zinc-800":"bg-white border-gray-200"}`}>
          {tabs.map(t=>(
            <button key={t.id} onClick={()=>{setTab(t.id);setMobileNav(false);}}
              className={`flex flex-col items-center py-2 rounded-sm text-[10px] ${tab===t.id?"bg-amber-400 text-zinc-950 font-bold":dark?"text-zinc-400":"text-gray-600"}`}>
      ...
      <main className="flex-1 p-3 sm:p-4 overflow-y-auto animate-fadeIn">

        {/* ── DASHBOARD ─────────────────────────────────────── */}
        {tab==="dashboard" && (
          <div className="space-y-6">
            <div>
              <div className={`text-[10px] uppercase tracking-widest mb-2 ${dimText2}`}>Markets</div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <div className={`text-[10px] uppercase tracking-widest mb-2 ${dimText2}`}>Latest News</div>
              ...
              <div className="space-y-3">
                <div className={`border rounded-sm p-3 ${cardBg}`}>
                  <div className={`text-[10px] uppercase tracking-widest mb-2 ${dimText2}`}>Portfolio</div>
                  {portfolioRows.length > 0 ? (
                    <><div className="flex justify-between text-xs mb-1"><span className={dimText}>Market Value</span><span className={`font-bold ${headText}`}>${fmt(totalMktVal)}</span></div>
                    <div className="flex justify-between text-xs"><span className={dimText}>Total P&L</span><span className={`font-bold ${clr(totalPnl)}`}>{totalPnl>=0?"+":""}${fmt(totalPnl)}</span></div></>
                  ) : <div className={`text-xs ${dimText3}`}>No holdings.</div>}
                </div>
                <div className={`border rounded-sm p-3 ${cardBg}`}>
                  <div className={`text-[10px] uppercase tracking-widest mb-2 ${dimText2}`}>Trade Journal</div>
                ...
                {rhStatus?.logged_in && rhPortfolio && (
                  <div className={`border rounded-sm p-3 ${cardBg}`}>
                    <div className={`text-[10px] uppercase tracking-widest mb-2 ${dimText2}`}>🪶 Robinhood</div>
        ...
        {/* ── CHARTS ────────────────────────────────────────── */}
        {tab==="charts" && (
          <RobinhoodChart initialTicker={chartTicker} dark={dark} BASE={BASE} onTickerChange={(t) => setChartTicker(t)} />
        )}

        {/* ── AI ────────────────────────────────────────────── */}
        {tab==="ai" && (
          <div className="space-y-4 max-w-3xl">
            <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>AI Stock Analysis · Powered by Claude</div>
            ...
            {aiLoading && <div className="border border-violet-800/50 bg-violet-950/10 rounded-sm p-8 text-center"><div className="text-violet-400 text-sm animate-pulse mb-2">🧠 Analyzing {aiTicker}...</div></div>}
            {aiResult && !aiLoading && !aiResult.error && (
              <div className="space-y-4">
                <div className="flex items-center gap-4 flex-wrap">
                  ...
                  <span className={`border rounded-sm px-3 py-1 text-sm font-bold uppercase ${sentimentColor(aiResult.sentiment)}`}>{aiResult.sentiment}</span>
                  <span className={`text-2xl font-bold ${scoreColor(aiResult.score)}`}>{aiResult.score}<span className={`text-sm ${dimText2}`}>/10</span></span>
                  {aiResult.recommendation && <span className={`border rounded-sm px-3 py-1 text-xs font-bold ${aiResult.recommendation==="BUY"?"border-emerald-600 text-emerald-400":aiResult.recommendation==="SELL"?"border-red-600 text-red-400":"border-amber-600 text-amber-400"}`}>{aiResult.recommendation}</span>}
                </div>
                ...
                {aiResult.price_target && (
                  <div className={`flex gap-6 text-xs border rounded-sm p-3 ${cardBg}`}>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {aiResult.bull_case && <div className="border border-emerald-800/30 bg-emerald-950/10 rounded-sm p-4"><div className="text-emerald-400 text-xs font-bold uppercase tracking-widest mb-2">🐂 Bull Case</div>{aiResult.bull_case.map((p,i)=><div key={i} className={`text-xs mb-1.5 leading-relaxed ${bodyText}`}>• {p}</div>)}</div>}
                  {aiResult.bear_case && <div className="border border-red-800/30 bg-red-950/10 rounded-sm p-4"><div className="text-red-400 text-xs font-bold uppercase tracking-widest mb-2">🐻 Bear Case</div>{aiResult.bear_case.map((p,i)=><div key={i} className={`text-xs mb-1.5 leading-relaxed ${bodyText}`}>• {p}</div>)}</div>}
                </div>
                {aiResult.technicals && <div className={`border rounded-sm p-4 ${cardBg}`}><div className={`text-xs font-bold uppercase tracking-widest mb-2 ${dimText}`}>Technical Analysis</div><p className={`text-xs leading-relaxed ${bodyText}`}>{aiResult.technicals}</p></div>}
              </div>
            )}
            {aiResult?.error && <div className="border border-red-800 bg-red-950/20 rounded-sm p-4 text-red-400 text-xs">{aiResult.error}</div>}
          </div>
        )}
        ...
        {tab==="options" && (
          <div className="space-y-4">
            ...
            <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Options Flow & Chain</div>
            <div className="flex gap-2 items-center flex-wrap">
              ...
              <div className="flex gap-0.5">
                <button onClick={()=>setOptType("CALL")} className={`px-3 py-1.5 text-xs rounded-sm ${optType==="CALL"?"bg-emerald-600 text-white font-bold":`border ${dark?"border-zinc-700 text-zinc-500":"border-gray-300 text-gray-500"}`}`}>CALLS</button>
                <button onClick={()=>setOptType("PUT")} className={`px-3 py-1.5 text-xs rounded-sm ${optType==="PUT"?"bg-red-600 text-white font-bold":`border ${dark?"border-zinc-700 text-zinc-500":"border-gray-300 text-gray-500"}`}`}>PUTS</button>
              </div>
        ...
        {tab==="robinhood" && (
          <div className="space-y-6">
            <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Brokerage Accounts</div>
            <div className="space-y-4">
              <div className="flex items-center gap-3 flex-wrap">
              ...
              {fidAccounts.length > 0 && (<div className="grid grid-cols-2 sm:grid-cols-3 gap-3">{fidAccounts.map((a:any)=>(<div key={a.key} onClick={()=>{setFidAccount(a.key);loadFidPortfolio(a.key);}} className={`border rounded-sm p-4 cursor-pointer transition-all ${fidAccount===a.key?`border-emerald-600 ${dark?"bg-emerald-950/20":"bg-emerald-50"}`:cardBg}`}><div className={`text-[10px] uppercase tracking-widest mb-1 ${dimText2}`}>{a.name}</div><div className={`text-xl font-bold font-mono ${headText}`}>${a.total_value?.toLocaleString("en-US",{minimumFractionDigits:2})}</div><div className={`text-sm font-mono font-bold ${a.total_gl>=0?"text-emerald-400":"text-red-400"}`}>{a.total_gl>=0?"+":""}${a.total_gl?.toLocaleString("en-US",{minimumFractionDigits:2})}</div><div className={`text-[10px] mt-1 ${dimText}`}>{a.holdings_count} positions</div></div>))}</div>)}
        ...
        {tab==="quotes" && (
          <div className="max-w-lg space-y-4">
            <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Live Quote Lookup</div>
        ...
        {tab==="watchlist" && (
          <div className="space-y-4 max-w-md">
            <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Watchlist</div>
            <div className="flex gap-2"><input value={wlInput} onChange={e=>setWlInput(e.target.value)} placeholder="Ticker" className={`flex-1 ${input} uppercase`} onKeyDown={e=>e.key==="Enter"&&addWatchlist()} /><button onClick={addWatchlist} className={btn}>ADD</button></div>
            {watchlist.map(t=>(<div key={t} className={`flex justify-between items-center border px-3 py-2 rounded-sm group ${cardBg}`}><div className="flex gap-6 items-center"><span className="text-amber-400 font-bold w-16 cursor-pointer hover:text-amber-300" onClick={()=>goToChart(t)}>{t}</span><span className={`font-mono ${headText}`}>{wlPrices[t]!=null?`$${fmt(wlPrices[t]!)}`:"—"}</span></div><button onClick={()=>removeWatchlist(t)} className={`${dimText3} hover:text-red-400 opacity-0 group-hover:opacity-100`}>✕</button></div>))}
            {!watchlist.length && <div className={`text-xs text-center py-8 ${dimText3}`}>Empty watchlist.</div>}
          </div>
        )}
        ...
        {tab==="portfolio" && (
          <div className="space-y-4">
            <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Portfolio — {portfolioName}</div>
            <div className="flex gap-2 flex-wrap">
              <input value={ptTicker} onChange={e=>setPtTicker(e.target.value)} placeholder="Ticker" className={`w-20 ${input} uppercase`} />
              <input value={ptShares} onChange={e=>setPtShares(e.target.value)} placeholder="Shares" className={`w-24 ${input}`} />
              <input value={ptCost} onChange={e=>setPtCost(e.target.value)} placeholder="Avg Cost" className={`w-28 ${input}`} />
              <button onClick={addPortfolio} className={btn}>ADD</button>
            </div>
            {portfolioRows.length > 0 && <div className={`flex gap-6 text-xs border rounded-sm p-3 ${cardBg}`}><div><span className={dimText}>MKT VALUE </span><span className={`font-bold ${headText}`}>${fmt(totalMktVal)}</span></div><div><span className={dimText}>P&L </span><span className={`font-bold ${clr(totalPnl)}`}>{totalPnl>=0?"+":""}${fmt(totalPnl)}</span></div></div>}
            <div className="overflow-x-auto"><table className="w-full text-xs border-collapse"><thead><tr className={`text-left border-b ${dark?"text-zinc-600 border-zinc-800":"text-gray-500 border-gray-200"}`}>{["Ticker","Shares","Cost","Price","Mkt Val","P&L","P&L%",""].map(h=><th key={h} className="py-2 px-2">{h}</th>)}</tr></thead><tbody>{portfolioRows.map(r=>(<tr key={r.ticker} className={`border-b ${dark?"border-zinc-900/50 hover:bg-zinc-900/50":"border-gray-100 hover:bg-gray-50"}`}><td className="py-2 px-2 text-amber-400 font-bold cursor-pointer" onClick={()=>goToChart(r.ticker)}>{r.ticker}</td><td className="py-2 px-2">{r.shares}</td><td className="py-2 px-2">${fmt(r.cost_basis)}</td><td className="py-2 px-2 font-mono">{r.price!=null?`$${fmt(r.price)}`:"—"}</td><td className="py-2 px-2">{r.mktVal!=null?`$${fmt(r.mktVal)}`:"—"}</td><td className={`py-2 px-2 font-bold ${clr(r.pnl)}`}>{r.pnl!=null?`${r.pnl>=0?"+":""}$${fmt(r.pnl)}`:"—"}</td><td className={`py-2 px-2 ${clr(r.pnlPct)}`}>{pct(r.pnlPct)}</td><td className="py-2 px-2"><button onClick={()=>deletePortfolio(r.ticker)} className={`${dimText3} hover:text-red-400`}>✕</button></td></tr>))}</tbody></table></div>
          </div>
        )}
        ...
        {tab==="journal" && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 flex-wrap">
              <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Trade Journal</div>
            ...
            {showTradeForm && (
              <div className={`border rounded-sm p-4 space-y-3 ${cardBg}`}>
            ...
            {trades.map(t=>(<div key={t.id} className={`border rounded-sm p-3 ${t.status==="open"?"border-amber-800/30":borderDim}`}><div className="flex items-center gap-3 flex-wrap"><span className="text-amber-400 font-bold cursor-pointer" onClick={()=>goToChart(t.ticker)}>{t.ticker}</span><span className={`text-[10px] px-1.5 py-0.5 rounded-sm border ${t.side==="long"?"border-emerald-700 text-emerald-400":"border-red-700 text-red-400"}`}>{t.side.toUpperCase()}</span><span className={`text-xs ${dimText}`}>Entry: ${fmt(t.entry_price)} ×{t.quantity}</span>{t.pnl!=null&&<span className={`text-xs font-bold ${clr(t.pnl)}`}>{t.pnl>=0?"+":""}${fmt(t.pnl)}</span>}<div className="ml-auto flex gap-1">{t.status==="open"&&<button onClick={()=>{const p=prompt("Exit price?");if(p)closeTrade(t.id,p);}} className="text-[10px] border border-emerald-800 text-emerald-600 px-2 py-0.5 rounded-sm">Close</button>}<button onClick={()=>deleteTrade(t.id)} className={`text-[10px] ${dimText3} hover:text-red-400 px-1`}>✕</button></div></div>{t.notes&&<p className={`text-[10px] mt-1 ${dimText2}`}>{t.notes}</p>}</div>))}
        ...
        {tab==="alerts" && (
          <div className="space-y-6 max-w-lg">
            <div>
              <div className={`text-[10px] uppercase tracking-widest mb-3 ${dimText2}`}>Price Alerts</div>
              ...
              {alerts.filter(a=>a.active).map(a=>(<div key={a.id} className={`flex justify-between items-center border px-3 py-2 rounded-sm group ${cardBg}`}><span className="text-amber-400 font-bold w-16">{a.ticker}</span><span className={`text-xs ${dimText}`}>{a.condition} ${fmt(a.price)}</span><button onClick={()=>deleteAlert(a.id)} className={`${dimText3} hover:text-red-400 opacity-0 group-hover:opacity-100`}>✕</button></div>))}
            </div>
            {triggered.length > 0 && <div><div className={`text-[10px] uppercase tracking-widest mb-2 ${dimText2}`}>Triggered</div>{triggered.slice(0,10).map((t,i)=>(<div key={i} className="flex gap-3 border border-emerald-900/50 bg-emerald-950/10 px-3 py-2 rounded-sm mb-1 text-xs"><span className="text-amber-400 font-bold w-16">{t.ticker}</span><span className={dimText}>{t.condition} ${fmt(t.target)} → ${fmt(t.actual_price)}</span></div>))}</div>}
            <div className={`border-t pt-4 ${borderDim}`}>
              <div className={`text-[10px] uppercase tracking-widest mb-2 ${dimText2}`}>News Keywords</div>
              ...
              <div className="flex flex-wrap gap-2">{keywords.map(k=>(<span key={k} className={`flex items-center gap-1 border px-2 py-1 rounded-sm text-xs ${cardBg}`}>{k}<button onClick={()=>removeKeyword(k)} className={`${dimText2} hover:text-red-400 ml-1`}>✕</button></span>))}</div>
            </div>
          </div>
        )}
        ...
        {tab==="news" && (
          <div className="space-y-4">
            <div className="flex items-center gap-3"><div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>News</div><button onClick={()=>fetch(`${BASE}/news/refresh`).then(()=>{loadNews();loadNewsAlerts();})} className={btnOutline}>↻ Refresh</button></div>
            {newsAlerts.length>0&&(<div className="border border-amber-800/30 bg-amber-950/10 rounded-sm p-3 space-y-2"><span className="text-xs text-amber-400 font-bold uppercase tracking-widest">Keyword Alerts</span>{newsAlerts.slice(0,5).map((a:any,i:number)=>(<div key={i} className="text-xs"><a href={a.link} target="_blank" rel="noreferrer" className={`hover:text-amber-300 ${bodyText}`}>{a.title}</a></div>))}</div>)}
            <div className="grid md:grid-cols-2 gap-x-6">{news.map((n,i)=>(<div key={i} className={`border-b py-3 ${borderDim2}`}><a href={n.link} target="_blank" rel="noreferrer" className={`text-xs hover:text-amber-300 leading-relaxed block mb-1 ${bodyText}`}>{n.title}</a><span className={`text-[10px] ${dimText2}`}>{n.source}</span></div>))}</div>
          </div>
        )}
            ...
            <div className="grid lg:grid-cols-4 gap-4">
              ...
              <div className="lg:col-span-3 space-y-3">
                {feedEntries.filter(e=>!feedFilter||e.source===feedFilter).filter(e=>!feedTickerFilter||e.tickers?.includes(feedTickerFilter)||e.title?.toUpperCase().includes(feedTickerFilter)).map((entry:any,i:number)=>(
                  <div key={i} className={`border rounded-sm p-3 hover:border-amber-500/30 transition-colors ${cardBg}`}>
                    ...
                    {entry.tickers?.length>0&&(<div className="flex gap-1.5 mt-2 flex-wrap">{entry.tickers.map((t:string)=>(<span key={t} className="border border-amber-800/30 bg-amber-950/10 px-1.5 py-0.5 rounded-sm text-[10px] text-amber-400 cursor-pointer hover:bg-amber-950/30" onClick={()=>goToChart(t)}>${t}</span>))}</div>)}
                  </div>
                ))}
        ...
        {tab==="screener" && (
          <div className="space-y-4">
            <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Stock Screener</div>
            ...
            {screenResults.length>0&&!screenLoading&&(
              <div className="overflow-x-auto"><table className="w-full text-xs border-collapse min-w-[800px]"><thead><tr className={`text-left border-b ${dark?"text-zinc-600 border-zinc-800":"text-gray-500 border-gray-200"}`}>
                {[{k:"name",l:"Ticker"},{k:"price",l:"Price"},{k:"change_pct",l:"Chg%"},{k:"market_cap",l:"MktCap"},{k:"pe",l:"P/E"},{k:"volume",l:"Vol"},{k:"dividend_yield",l:"Div%"},{k:"from_52h_pct",l:"vs52H"}].map(col=>(
                ...
                {screenResults.map(r=>(<tr key={r.ticker} className={`border-b ${dark?"border-zinc-900/30 hover:bg-zinc-900/50":"border-gray-100 hover:bg-gray-50"}`}>
        ...
        {tab==="convictions" && (
          <div className="space-y-4">
            <div className="flex items-center gap-2 flex-wrap">
              <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Convictions</div>
              {["all","watching","entered","closed"].map(s=>(<button key={s} onClick={()=>setConvFilter(s)} className={`px-2 py-1 text-xs rounded-sm ${convFilter===s?"bg-amber-400 text-zinc-950 font-bold":`border ${dark?"border-zinc-800 text-zinc-500":"border-gray-300 text-gray-500"}`}`}>{s}</button>))}
              <button onClick={()=>setShowConvForm(v=>!v)} className={`ml-auto ${btnOutline}`}>+ Add</button>
              {convictions.filter(c=>c.trader==="Gaetano").length===0&&<button onClick={importGaetano} className="text-xs border border-amber-900 px-3 py-1 text-amber-600 rounded-sm">Import Gaetano</button>}
            </div>
            ...
            {filteredConvictions.map(c=>(<div key={c.id} className={`border rounded-sm p-3 ${c.status==="entered"?"border-emerald-800/30":borderDim}`}><div className="flex items-start justify-between gap-2"><div className="flex items-center gap-2 flex-wrap"><span className="text-amber-400 font-bold cursor-pointer" onClick={()=>goToChart(c.ticker)}>{c.ticker}</span><span className={`text-xs ${dimText2}`}>@{c.trader}</span><span className={`text-[10px] border rounded-sm px-1 ${c.status==="entered"?"border-emerald-700 text-emerald-400":"border-amber-700 text-amber-500"}`}>{c.status}</span></div><div className="flex gap-1 shrink-0">{c.status!=="entered"&&<button onClick={()=>updateConvStatus(c.id,"entered")} className="text-[10px] border border-emerald-800 text-emerald-600 px-2 py-0.5 rounded-sm">Enter</button>}{c.status!=="closed"&&<button onClick={()=>updateConvStatus(c.id,"closed")} className={`text-[10px] border px-2 py-0.5 rounded-sm ${dark?"border-zinc-700 text-zinc-600":"border-gray-300 text-gray-500"}`}>Close</button>}<button onClick={()=>deleteConviction(c.id)} className={`text-[10px] ${dimText3} hover:text-red-400 px-1`}>✕</button></div></div>{c.thesis&&<p className={`text-[11px] mt-1 leading-relaxed ${dimText}`}>{c.thesis}</p>}</div>))}
          </div>
        )}
        ...
        {tab==="heatmap" && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 flex-wrap">
              <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Market Heat Map</div>
            ...
            {heatmapData.length > 0 && (<div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-1">{heatmapData.sort((a:any,b:any)=>(b.market_cap||0)-(a.market_cap||0)).map((s:any)=>{const p=s.change_pct||0;const bg=p>3?"bg-emerald-600":p>1.5?"bg-emerald-700":p>0?"bg-emerald-900":p>-1.5?"bg-red-900":p>-3?"bg-red-700":"bg-red-600";return(<div key={s.ticker} onClick={()=>goToChart(s.ticker)} className={`${bg} rounded-sm p-2 cursor-pointer hover:opacity-80 text-center`}><div className="text-[11px] font-bold text-white">{s.ticker}</div><div className="text-[10px] font-mono text-white/80">{p>=0?"+":""}{p.toFixed(1)}%</div><div className="text-[9px] text-white/50">${s.price}</div></div>);})}</div>)}
        ...
        {tab==="divs" && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 flex-wrap">
              <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Dividend Tracker</div>
            ...
            {divData && !divLoading && (<div className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <div className={`border rounded-sm p-3 ${cardBg}`}><div className={`text-[10px] ${dimText2}`}>Annual Income</div><div className="text-xl font-bold font-mono text-emerald-400">${divData.total_annual_income?.toFixed(2)}</div></div>
                <div className={`border rounded-sm p-3 ${cardBg}`}><div className={`text-[10px] ${dimText2}`}>Monthly</div><div className={`text-lg font-bold font-mono ${headText}`}>${(divData.total_annual_income/12)?.toFixed(2)}</div></div>
                <div className={`border rounded-sm p-3 ${cardBg}`}><div className={`text-[10px] ${dimText2}`}>Payers</div><div className="text-lg font-bold font-mono text-amber-400">{divData.holdings?.filter((h:any)=>h.yield_pct>0).length}/{divData.holdings?.length}</div></div>
              </div>
        ...
        {tab==="perf" && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 flex-wrap">
              <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Portfolio vs SPY</div>
              {fidAccounts.length > 0 && (<select value={fidAccount} onChange={e=>setFidAccount(e.target.value)} className={input}>{fidAccounts.map((a:any)=><option key={a.key} value={a.key}>{a.name}</option>)}</select>)}
            ...
            {perfData && !perfLoading && !perfData.error && (<div className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className={`border rounded-sm p-3 ${cardBg}`}><div className={`text-[10px] ${dimText2}`}>Cost Basis</div><div className={`text-lg font-bold font-mono ${headText}`}>${perfData.total_cost?.toLocaleString("en-US",{minimumFractionDigits:0})}</div></div>
                <div className={`border rounded-sm p-3 ${cardBg}`}><div className={`text-[10px] ${dimText2}`}>Current</div><div className={`text-lg font-bold font-mono ${headText}`}>${perfData.current_value?.toLocaleString("en-US",{minimumFractionDigits:0})}</div></div>
                <div className={`border rounded-sm p-3 ${cardBg}`}><div className={`text-[10px] ${dimText2}`}>P&L</div><div className={`text-lg font-bold font-mono ${(perfData.current_value-perfData.total_cost)>=0?"text-emerald-400":"text-red-400"}`}>{(perfData.current_value-perfData.total_cost)>=0?"+":""}${(perfData.current_value-perfData.total_cost)?.toLocaleString("en-US",{minimumFractionDigits:0})}</div></div>
        ...
        {tab==="backtest" && (
          <div className="space-y-4 max-w-4xl">
            <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Strategy Backtester</div>
            <div className="flex gap-2 flex-wrap items-end">
              ...
              <div><div className={`text-[10px] mb-1 ${dimText2}`}>Strategy</div><select value={btStrategy} onChange={e=>setBtStrategy(e.target.value)} className={input}><option value="sma_cross">SMA Cross</option><option value="rsi">RSI</option></select></div>
        ...
        {tab==="analytics" && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 flex-wrap">
              <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Portfolio Analytics</div>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    ...
                    <div key={s.l} className={`border rounded-sm p-3 ${cardBg}`}><div className={`text-[10px] uppercase tracking-widest mb-1 ${dimText2}`}>{s.l}</div><div className={`text-lg font-bold font-mono ${s.c}`}>{s.v}</div></div>
                  ))}
                </div>
                <div className="flex gap-4 flex-wrap">
                  ...
                  <div className={`border rounded-sm p-3 ${cardBg}`}><div className={`text-[10px] ${dimText2}`}>Worst</div><span className="text-red-400 font-bold">{analyticsData.stats?.worst_performer?.ticker}</span> <span className="text-red-400 text-xs">{analyticsData.stats?.worst_performer?.pnl_pct?.toFixed(1)}%</span></div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {analyticsData.sectors?.map((s:any)=>(
                      <div key={s.sector} className={`border rounded-sm p-3 ${cardBg}`}>
        ...
        {tab==="optcalc" && (
          <div className="space-y-4 max-w-4xl">
            <div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Options P&L Calculator</div>
            <div className="flex gap-2 flex-wrap items-end">
            ...
            {optCalcResult && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className={`border rounded-sm p-3 ${cardBg}`}><div className={`text-[10px] ${dimText2}`}>Breakeven</div><div className={`text-lg font-bold font-mono ${headText}`}>${optCalcResult.breakeven.toFixed(2)}</div></div>
                  <div className={`border rounded-sm p-3 ${cardBg}`}><div className={`text-[10px] ${dimText2}`}>Max Loss</div><div className="text-lg font-bold font-mono text-red-400">${Math.abs(optCalcResult.maxLoss).toFixed(2)}</div></div>
                  <div className={`border rounded-sm p-3 ${cardBg}`}><div className={`text-[10px] ${dimText2}`}>Max Profit</div><div className="text-lg font-bold font-mono text-emerald-400">{optCalcResult.maxProfit===Infinity?"Unlimited":`$${optCalcResult.maxProfit.toFixed(2)}`}</div></div>
                  <div className={`border rounded-sm p-3 ${cardBg}`}><div className={`text-[10px] ${dimText2}`}>Cost Basis</div><div className={`text-lg font-bold font-mono ${headText}`}>${optCalcResult.costBasis.toFixed(2)}</div></div>
                </div>
                ...
                <div className={`border rounded-sm p-1 ${cardBg}`}>
                  <canvas ref={el=>{
        ...
        {tab==="earnings" && (
          <div className="space-y-4 max-w-2xl">
            <div className="flex items-center gap-3"><div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Upcoming Earnings</div><button onClick={loadEarnings} className={btnOutline}>↻ Refresh</button></div>
            {earnings.map((e,i)=>(<div key={i} className={`flex items-center gap-3 px-3 py-2 rounded-sm border text-xs ${cardBg}`}><span className={`w-24 font-mono ${dimText}`}>{e.date}</span><span className="text-amber-400 font-bold w-16 cursor-pointer" onClick={()=>goToChart(e.symbol)}>{e.symbol}</span><span className={`flex-1 truncate ${dimText}`}>{e.hour==="bmo"?"Before Open":e.hour==="amc"?"After Close":""}</span><span className={dimText}>EPS: {e.epsEstimate||"—"}</span><button onClick={()=>{setTab("ai");setAiTicker(e.symbol);setTimeout(()=>runAI(e.symbol),100);}} className="text-violet-400 text-[10px]">🧠</button></div>))}
            <button onClick={() => setTab("risk")} style={{ padding: "6px 12px", fontSize: 11, cursor: "pointer", border: "none", borderRadius: 6, background: tab==="risk" ? (dark ? "#ffffff15" : "#e2e8f0") : "transparent", color: tab==="risk" ? (dark ? "#fff" : "#111") : (dark ? "#888" : "#666"), fontWeight: tab==="risk" ? 700 : 500, fontFamily: "inherit", transition: "all 0.15s" }}>Risk</button>
            <button onClick={() => setTab("flow")} style={{ padding: "6px 12px", fontSize: 11, cursor: "pointer", border: "none", borderRadius: 6, background: tab==="flow" ? (dark ? "#ffffff15" : "#e2e8f0") : "transparent", color: tab==="flow" ? (dark ? "#fff" : "#111") : (dark ? "#888" : "#666"), fontWeight: tab==="flow" ? 700 : 500, fontFamily: "inherit", transition: "all 0.15s" }}>Flow</button>
            <button onClick={() => setTab("earnings-intel")} style={{ padding: "6px 12px", fontSize: 11, cursor: "pointer", border: "none", borderRadius: 6, background: tab==="earnings-intel" ? (dark ? "#ffffff15" : "#e2e8f0") : "transparent", color: tab==="earnings-intel" ? (dark ? "#fff" : "#111") : (dark ? "#888" : "#666"), fontWeight: tab==="earnings-intel" ? 700 : 500, fontFamily: "inherit", transition: "all 0.15s" }}>Earnings</button>
            <button onClick={() => setTab("filings")} style={{ padding: "6px 12px", fontSize: 11, cursor: "pointer", border: "none", borderRadius: 6, background: tab==="filings" ? (dark ? "#ffffff15" : "#e2e8f0") : "transparent", color: tab==="filings" ? (dark ? "#fff" : "#111") : (dark ? "#888" : "#666"), fontWeight: tab==="filings" ? 700 : 500, fontFamily: "inherit", transition: "all 0.15s" }}>Filings</button>
            <button onClick={() => setTab("econ")} style={{ padding: "6px 12px", fontSize: 11, cursor: "pointer", border: "none", borderRadius: 6, background: tab==="econ" ? (dark ? "#ffffff15" : "#e2e8f0") : "transparent", color: tab==="econ" ? (dark ? "#fff" : "#111") : (dark ? "#888" : "#666"), fontWeight: tab==="econ" ? 700 : 500, fontFamily: "inherit", transition: "all 0.15s" }}>Econ</button>
            <button onClick={() => setTab("credit")} style={{ padding: "6px 12px", fontSize: 11, cursor: "pointer", border: "none", borderRadius: 6, background: tab==="credit" ? (dark ? "#ffffff15" : "#e2e8f0") : "transparent", color: tab==="credit" ? (dark ? "#fff" : "#111") : (dark ? "#888" : "#666"), fontWeight: tab==="credit" ? 700 : 500, fontFamily: "inherit", transition: "all 0.15s" }}>Macro</button>
            <button onClick={() => setTab("transcripts")} style={{ padding: "6px 12px", fontSize: 11, cursor: "pointer", border: "none", borderRadius: 6, background: tab==="transcripts" ? (dark ? "#ffffff15" : "#e2e8f0") : "transparent", color: tab==="transcripts" ? (dark ? "#fff" : "#111") : (dark ? "#888" : "#666"), fontWeight: tab==="transcripts" ? 700 : 500, fontFamily: "inherit", transition: "all 0.15s" }}>Calls</button>
            {!earnings.length&&<div className={`text-xs text-center py-8 ${dimText3}`}>Click Refresh</div>}
          </div>
        )}
        ...
        {tab==="calendar" && (
          <div className="space-y-4 max-w-2xl">
            <div className="flex items-center gap-3"><div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Economic Calendar</div><button onClick={()=>fetch(`${BASE}/calendar/refresh`).then(()=>loadCalendar())} className={btnOutline}>↻ Refresh</button></div>
            {calendar.map((ev,i)=>{const today=new Date().toISOString().slice(0,10);const isToday=ev.date===today;return(<div key={i} className={`flex items-center gap-3 px-3 py-2 rounded-sm border text-xs ${isToday?"border-red-600/50 bg-red-950/20":cardBg}`}><span className={`w-24 font-mono ${dimText}`}>{ev.date}</span><span className={`border rounded-sm px-1 text-[10px] shrink-0 ${catColor(ev.category)}`}>{ev.category}</span><span className={`flex-1 ${bodyText}`}>{ev.event}</span>{isToday&&<span className="text-red-400 font-bold animate-pulse text-[10px]">TODAY</span>}</div>);})}
        ...
        {tab==="macro" && (
          <div className="space-y-4">
            <div className="flex items-center gap-3"><div className={`text-[10px] uppercase tracking-widest ${dimText2}`}>Macro Dashboard</div><button onClick={loadMacro} className={btnOutline}>↻ Refresh</button></div>
      ...
      <footer className={`border-t px-4 py-1.5 flex justify-between text-[10px] ${dark?"border-zinc-800/50 text-zinc-700":"border-gray-200 text-gray-400"}`}>
        <span>STOCK TERMINAL v4.4</span>
